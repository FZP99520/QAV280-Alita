...
GPS_BUFF gps;
nmea_msg gps_data;
Position_TypeDef GPS_Position;
void GPS_Show()
{
	if(gps.rx_ok==1)
	{
		gps.rx_ok=0;
		GPS_Analysis(&gps_data,(&gps)->buff);//Analysis gps data
		if(gps_data.gpssta==0)
		{
		  printf("Location uncertain !\n");
			return;
		}
	  else if(gps_data.fixmode==2)
		{
			printf("Location mode :2D\n");
		}
		else if(gps_data.fixmode==3)
		{
			printf("Location mode :3D\n");
		}
		else ;
		if(gps_data.fixmode!=1)
		{
			printf("longitude:%f\n",(float)gps_data.longitude/100000);
			printf("latitude:%f\n",(float)gps_data.latitude/100000);
			printf("altitude:%.2fm   posslnum:%d   svnum=%d\n",\
			(float)gps_data.altitude/10,gps_data.posslnum,gps_data.svnum);
		}
	}
...
void NMEA_GPGGA_Analysis(nmea_msg *gpsx,u8 *buf)
{
	...
	if(posx!=0XFF)gpsx->altitude=NMEA_Str2num(p1+posx,&dx)/10.0f;  
}
//分析GPGSA信息
//gpsx:nmea信息结构体
//buf:接收到的GPS数据缓冲区首地址
...
void NMEA_GPRMC_Analysis(nmea_msg *gpsx,u8 *buf)
{
	...
	if(posx!=0XFF)
	{
		gpsx->mag_angle=NMEA_Str2num(p1+posx,&dx);		 				
	} 
}
//分析GPVTG信息
//gpsx:nmea信息结构体
...
void NMEA_GPVTG_Analysis(nmea_msg *gpsx,u8 *buf)
{
	...
	if(posx!=0XFF)
	{
		gpsx->navigate_angle=NMEA_Str2num(p1+posx,&dx);
		gpsx->navigate_angle/=NMEA_Pow(10,dx);	
	}
	posx=NMEA_Comma_Pos(p1,7);								//得到地面速率
	if(posx!=0XFF)
	{
		gpsx->speed=NMEA_Str2num(p1+posx,&dx);
		if(dx<3)gpsx->speed*=NMEA_Pow(10,3-dx)/1000.0f;	//缩小1000倍，因为解析数据时被放大1000倍
	}
}  
//提取NMEA-0183信息
//gpsx:nmea信息结构体
...
{
	static u8 cnt=0;
	static double latitude_sum=0,longitude_sum=0;
	...
	memset(buf,0,USART2_Buff_Len);   //Clear buff
	if(gpsx->gpssta!=0) 
	{
		if(GPS_Position.Init_Sta==0)
		{
		  cnt++;
		  latitude_sum+=gpsx->latitude;
		  longitude_sum+=gpsx->longitude;
		  altitude_sum+=gpsx->altitude;
		  if(cnt==20)
		  {
			  GPS_Position.Org_latitude=latitude_sum/cnt;
			  GPS_Position.Org_longitude=longitude_sum/cnt;
			  GPS_Position.Org_altitude=altitude_sum/cnt;
			  latitude_sum=0;
			  longitude_sum=0;
			  altitude_sum=0;
				GPS_Position.Init_Sta=1;//init finish
		  }
	 }
		if(GPS_Position.Init_Sta==1)
		{
			GPS_Position.Now_latitude=gpsx->latitude;
			GPS_Position.Now_longitude=gpsx->longitude;
			GPS_Position.Now_altitude=gpsx->altitude;
			GPS_Position.delta_Y=(GPS_Position.Now_latitude-GPS_Position.Org_latitude)*111000.0f;
			GPS_Position.delta_x=(GPS_Position.Now_longitude-GPS_Position.Org_longitude)*111000.0f;
			GPS_Position.delta_Z=GPS_Position.Now_altitude-GPS_Position.Org_altitude;
		}
	}
}
...
{			 
	u16 len=0,i;
	u8 rval=0;
	while((gps.rx_ok==1) && len<100)//等待接收到应答   
	{
		len++;
		Delay_ms(5);
	}		 
	if(len<250)   	//超时错误.
	{
		...
		for(i=0;i<len;i++)if(gps.buff[i]==0XB5)break;//查找同步字符 0XB5
		if(i==len)rval=2;						//没有找到同步字符
		else if(gps.buff[i+3]==0X00)rval=3;//接收到NACK应答
...
{
	u8 i;
	_ublox_cfg_cfg *cfg_cfg=(_ublox_cfg_cfg *)USART1_TX_Buff;
	...
	USART1_SendData(USART1_TX_Buff,sizeof(_ublox_cfg_cfg));	//通过dma发送出去
...
{
	_ublox_cfg_msg *cfg_msg=(_ublox_cfg_msg *)USART1_TX_Buff;
	...
	while(DMA1_Channel7->CNDTR!=0);	//等待通道7传输完成   
	USART1_SendData(USART1_TX_Buff,sizeof(_ublox_cfg_msg));	//通过dma发送出去
...
{
	_ublox_cfg_prt *cfg_prt=(_ublox_cfg_prt *)USART1_TX_Buff;
	...
	USART1_SendData(USART1_TX_Buff,sizeof(_ublox_cfg_prt));	//通过dma发送出去
	Delay_ms(200);				//等待发送完成 
	USART1_Init(baudrate);	//重新初始化串口2   
...
{
	_ublox_cfg_tp *cfg_tp=(_ublox_cfg_tp *)USART1_TX_Buff;
	...
	while(DMA1_Channel4->CNDTR!=0);	//等待通道7传输完成   
	USART1_SendData(USART1_TX_Buff,sizeof(_ublox_cfg_tp));	//通过dma发送出去
	return Ublox_Cfg_Ack_Check();
}
//配置UBLOX NEO-6的更新速率	    
...
{
	_ublox_cfg_rate *cfg_rate=(_ublox_cfg_rate *)USART1_TX_Buff;
	...
	while(DMA1_Channel4->CNDTR!=0);	//等待通道7传输完成   
	USART1_SendData(USART1_TX_Buff,sizeof(_ublox_cfg_rate));//通过dma发送出去
	return Ublox_Cfg_Ack_Check();
}
